...
def authorizeOrigination(voiceConnectorId, elasticIP):
    ...
    response = chime.put_voice_connector_origination(
        ...
        Origination={
            "Routes": [
                {"Host": elasticIP, "Port": 5060, "Protocol": "UDP", "Priority": 1, "Weight": 1},
            ],
            "Disabled": False,
        },
    )
...
def createVoiceConnector(region):
    ...
    response = chime.create_voice_connector(
        Name="Trunk" + str(uuid.uuid1()), AwsRegion="us-east-1", RequireEncryption=False
    )
...
def associatePhoneNumber(voiceConnector, phoneNumber):
...
def on_create(event):
    ...
    if streaming == "true":
        ...
        voiceConnector = createVoiceConnector(region)
        voiceConnector = associatePhoneNumber(voiceConnector, newPhoneNumber)
        authorizeTermination(voiceConnector["voiceConnectorId"], elasticIP)
