...
export class AsteriskEndpoint extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const chimeSecurityGroup = new ec2.SecurityGroup(
      this,
      "ChimeSecurityGroup",
      {
        vpc,
    ...
    chimeSecurityGroup.addIngressRule(
    ...
    asteriskRole.addManagedPolicy(
      iam.ManagedPolicy.fromAwsManagedPolicyName("AmazonSSMManagedInstanceCore")
    );

    const createVoiceConnectorRole = new iam.Role(
      this,
    ...
    const createVoiceConnectorLambda = new lambda.Function(
      this,
      "createVCLambda",
      {
        code: lambda.Code.fromAsset("src/asteriskResources", {
          exclude: ["**", "!createVoiceConnector.py"],
        }),
    ...
    const voiceConnectorProvider = new custom.Provider(
      this,
      "voiceConnectorProvider",
      {
        onEventHandler: createVoiceConnectorLambda,
      }
    );

    const voiceConnectorResource = new CustomResource(
      this,
    ...
    const voiceConnectorId =
      voiceConnectorResource.getAttString("voiceConnectorId");
    const outboundHostName =
      voiceConnectorResource.getAttString("outboundHostName");
    ...
    const voiceConnectorParameter = new ssm.StringParameter(
      this,
      "voiceConnector",
      {
    ...
    const outboundHostNameParameter = new ssm.StringParameter(
      this,
      "outboundHostName",
      {
    ...
    const asteriskConfig = new Asset(this, "AsteriskConfig", {
      path: path.join(__dirname, "../src/asteriskResources/config.sh"),
    });
    ...
    //   value:
    //     "aws secretsmanager get-secret-value --secret-id ec2-ssh-key/audioCode-keypair/private --query SecretString --output text > cdk-key.pem && chmod 400 cdk-key.pem",
    // });
    // new cdk.CfnOutput(this, "ssh command", {
    //   value:
    //     "ssh -i cdk-key.pem -o IdentitiesOnly=yes ec2-user@" +
    //     asteriskInstnace.instancePublicIp,
    // });
    new cdk.CfnOutput(this, "PhoneNumber", { value: phoneNumber }),
      new cdk.CfnOutput(this, "VoiceConnector", { value: outboundHostName });
    new cdk.CfnOutput(this, "AsteriskPublicIP", {
      value: asteriskInstnace.instancePublicIp,
    });
  }
}
